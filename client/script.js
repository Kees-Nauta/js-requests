...
sayHelloButton.addEventListener(`mouseover`, changeToBlack)
...
const repeatMyParam = () => {
    const parmam = `hello`
    axios.get(`http://localhost:3000/repeat/parmam`)
    .then(res =>{
        console.log(res.data)
...
const sendQuery = () =>{
    const qParam = `test query`
    axios.get(`http://localhost:3000/query-test?${qParam}`)
    .then(res =>{
        console.log(res.data)
    })
}
...
/*
    ...
    you sent and empty query
    2: Send more than 1 query on the URL -- what happened? 
    you sent more than one query!
*/
...
const postFood = (event) =>{
    ...
    .then(res => {
        ...
        resDisplay.textContent = res.data
...
document.getElementById(`foodForm`).addEventListener(`submit`, postFood)